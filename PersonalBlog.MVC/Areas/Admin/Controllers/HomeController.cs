...
namespace PersonalBlog.MVC.Areas.Admin.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public HomeController(ISkillService skillService, IHomePageSliderService homePageSliderService, IMessageService messageService, IEducationService educationService, IExperienceService experienceService, IInterestedsService interestedService, IArticleService articleService, ICategoryService categoryService, IAdminService adminService)
        {
        ...
        public async Task<IActionResult> EditAdmin(AdminUpdateDto adminUpdateDto,string oldPsswrd, string oldSQHash)
        {
            ...
            {
                if (oldPsswrd != adminUpdateDto.Password)
                {
                    using var md5 = MD5.Create();
                    var result1 = md5.ComputeHash(Encoding.ASCII.GetBytes(adminUpdateDto.Password));
                    var strResult = BitConverter.ToString(result1);
                    adminUpdateDto.Password = strResult.Replace("-", "");
                }
                if (oldSQHash != adminUpdateDto.SQAnswer)
                {
                    using var md5New = MD5.Create();
                    var result2 = md5New.ComputeHash(Encoding.ASCII.GetBytes(adminUpdateDto.SQAnswer));
                    var strResult2 = BitConverter.ToString(result2);
                    adminUpdateDto.SQAnswer = strResult2.Replace("-","");
                }
                await _adminService.Update(adminUpdateDto, "Evren Uludağ");
